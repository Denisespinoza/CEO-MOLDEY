...
export async function generateOrderNumber(): Promise<string> {
  ...
  if (!data) return 'MLD-0001';
  const prefix = data.order_number.startsWith('MLD') ? 'MLD' : 'MLD';
...
export async function createOrder(order: Partial<Order>, userId?: string): Promise<Order> {
  ...
  // If the order has multiple articles, derive totals + summary from items
  ...
  // Top-level summary fields (first item) keep existing views working
  const first = hasItems ? items[0] : null;
  const summaryGarment = first ? first.garment_type : (order.garment_type || '');
  const summaryArticle = first
    ? (items.length > 1 ? `${first.article_name || first.garment_type} (+${items.length - 1} más)` : (first.article_name || first.garment_type))
    : (order.article_name || order.garment_type || '');
  const summarySizes = first ? first.sizes : (order.sizes || '');
  const summaryFabric = first ? first.fabric_type : (order.fabric_type || '');
  const summaryWork = first ? first.work_type : (order.work_type || '');

  const remaining_balance = totalPrice - (Number(order.paid_amount) || 0);
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      garment_type: summaryGarment,
      article_name: summaryArticle,
      sizes: summarySizes,
      quantity: totalQuantity,
      fabric_type: summaryFabric,
      work_type: summaryWork,
      notes: order.notes || '',
  ...
  // Track garment types: all items, or the single garment
  if (hasItems) {
    for (const it of items) {
      if (it.garment_type) await incrementGarmentType(it.garment_type);
    }
  } else if (order.garment_type) {
