...
import type { CatalogItem, CatalogProductImage, CatalogStatus, Category } from './types';
...
// ─────────────────────────────────────────────────────────────
// GALLERY: catalog_product_images (multiple photos per product)
// ─────────────────────────────────────────────────────────────

// Get all gallery images for a product, ordered
export async function getProductImages(productId: string): Promise<CatalogProductImage[]> {
  const { data, error } = await supabase
...
// Add a new gallery image (uploads to R2 first, then inserts row)
export async function addProductImage(productId: string, file: File): Promise<CatalogProductImage> {
  if (file.size > MAX_UPLOAD_BYTES) {
    throw new Error(`El archivo pesa ${(file.size / 1024 / 1024).toFixed(1)}MB. El máximo permitido es 5MB.`);
  }

  const compressed = await compressImage(file);
  const path = `catalog/${productId}/gallery_${Date.now()}.jpg`;
  ...
  const nextOrder = existing.length > 0 ? Math.max(...existing.map(i => i.sort_order)) + 1 : 0;
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      is_main: false, // gallery photos are additional; main photo stays as photo_url
    })
    .select()
    .single();
...
// Set an image as the main one (unsets all others, syncs photo_url)
export async function setMainProductImage(productId: string, imageId: string, imageUrl: string): Promise<void> {
  // Unset all
  await supabase
    .from('catalog_product_images')
  ...
  // Sync as product's main photo_url
  await supabase
    .from('internal_catalog')
...
// Reorder a gallery image (move up/down by swapping sort_order)
export async function updateImageSortOrder(imageId: string, newOrder: number): Promise<void> {
  const { error } = await supabase
    .from('catalog_product_images')
