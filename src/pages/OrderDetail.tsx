                ...
                {order.items && order.items.length > 1 ? (
                  ...
                  <div className="space-y-2">
                    ...
                    {order.items.map((it, i) => (
                      <div key={i} className="rounded-lg border border-petrol-200 dark:border-slate-600 bg-white/60 dark:bg-slate-700/30 p-3">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-semibold text-petrol-800 dark:text-white">
                            {i + 1}. {it.article_name || it.garment_type || 'Sin nombre'}
                          </span>
                          <span className="text-sm font-bold text-violet-600 dark:text-violet-400">
                            ${Number(it.price).toLocaleString('es-AR')}
                          </span>
                        </div>
                        <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-petrol-500 dark:text-petrol-400">
                          {it.garment_type && <span>Molde: <span className="text-petrol-700 dark:text-petrol-300">{it.garment_type}</span></span>}
                          {it.sizes && <span>Talles: <span className="text-petrol-700 dark:text-petrol-300">{it.sizes}</span></span>}
                          <span>Cant: <span className="text-petrol-700 dark:text-petrol-300">{it.quantity}</span></span>
                          {it.fabric_type && <span>Tela: <span className="text-petrol-700 dark:text-petrol-300">{it.fabric_type}</span></span>}
                          {it.work_type && <span className="text-violet-500">{it.work_type}</span>}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                      <p className="text-xs text-petrol-500 dark:text-petrol-400">Molde / Código</p>
