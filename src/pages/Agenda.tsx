...
export default function Agenda() {
  ...
  const loadAgenda = useCallback(async () => {
    ...
    try {
      const [eventData, catalogData] = await Promise.all([
        // Kanban carga todos los eventos sin filtro de fecha
        viewMode === 'kanban'
          ? getAgendaEventsAll()
  ...
  }, [canUseAgenda, rangeFromIso, rangeToIso, viewMode]);

  useEffect(() => {
    loadAgenda();
  }, [loadAgenda]);

  // Suscripción realtime: se actualiza automáticamente cuando cambia agenda_events
  useEffect(() => {
    if (!canUseAgenda) return;
    const channel = supabase
      .channel('agenda_events_realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'agenda_events' }, () => {
        loadAgenda();
      })
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [canUseAgenda, loadAgenda]);
        ...
        <div className="relative p-6 md:p-8">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-slate-300">Planificación operativa de Moldey</p>
            </div>
